...
5つの統一されたシナリオで評価した結果、**sonnetモデルのみが実用レベル**に達しました：
...
### 📋 実験内容
#### 最新ベンチマーク（Issues #22-#28）
5つのモデルに対して標準化された5シナリオ（基本ファイル操作、テキスト検索、ファイル作成、ワークフロー解析、複合タスク）を実施。
...
2. **時間認識の問題** - glm-4.5は32倍の時間測定誤差
3. **「成功の偽装」問題** - gemini-2.5-proは失敗を成功として報告
4. **データ不正確性** - gpt-oss-120bは48%の過少報告
...
3. **避けるべきモデル**:
   - gemini-2.5-pro（「成功の偽装」問題 - 最も危険）
   - gpt-oss-120b（データ不正確性）
   - glm-4.5（時間認識の欠如）
   - gpt-5（シナリオ5で未完了）
   - kimi-k2（ツール使用非対応）※初期実験
   - qwen3-coder（互換性なし）※初期実験
   - gpt-5-nano（無限ループ問題）※初期実験
...
**最終更新**: 2025年8月9日
**最新ベンチマーク**: Issues #22-#28（5モデル、標準化5シナリオ）
