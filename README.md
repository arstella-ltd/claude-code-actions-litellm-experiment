...
| **gpt-oss-120b** | $0.090 | $0.450 | 🔥 高コスパ | 高速かつ安価 |
| **kimi-k2** | $0.140 | $2.490 | ⚠️ 出力高価 | 入力は安いが出力は高価 |
| **glm-4.5** | $0.200 | $0.800 | ✅ バランス型 | コストと性能の最適バランス |
| **qwen3-coder** | $0.200 | $0.800 | ✅ バランス型 | glm-4.5と同価格 |
| **gpt-5-mini** | $0.250 | $2.000 | ⚠️ 中価格 | 高機能但しそれなりの価格 |
...
2. **gpt-oss-120b**: 速度とコストのバランスが良い（入力$0.09/1M、出力$0.45/1M）

**バランス型モデル**:
- **glm-4.5**: 実用的な性能と合理な価格（入力$0.20/1M、出力$0.80/1M）
- **qwen3-coder**: glm-4.5と同価格帯で選択肢となる

**価格注意点**:
- **kimi-k2**: 入力は安いが出力が非常に高価（$2.49/1M）
- **gpt-5**: プレミアム価格ながら実験では失敗（入力$1.25/1M、出力$10.00/1M）

#### 💰 推奨モデル選択ガイド

| 利用シーン | 推奨モデル | 理由 |
|-----------|-----------|------|
| **コスト重視** | gpt-5-nano | 圧倒的な安さ、簡単なタスクに最適 |
| **速度+コスト** | gpt-oss-120b | 高速実行（28.1秒）と低価格の両立 |
| **バランス重視** | glm-4.5 | 高品質な出力と合理価格の最適バランス |
| **高性能重視** | gpt-5-mini | 最高品質の分析、それなりの価格を許容できる場合 |
| **避けたほうが良い** | gpt-5, kimi-k2 | 価格対効果が悪い、または互換性問題 |
