...
**動作確認済みモデル**:
- Claude ファミリー（claude-sonnet-4-20250514 など）

**互換性に問題があるモデル**:
- `qwen3-coder` - レスポンスが空になる（`output_tokens: 0`）
- 他の非 Claude モデル - Claude Code Action の期待する形式と異なる可能性

**推奨事項**:
1. Claude Code Action を使用する場合は、Claude モデルを使用することを強く推奨
2. LiteLLM 経由で他のモデルを使用する場合は、事前に互換性テストを実施
3. エラー時はワークフローログでデバッグ出力を確認
