...
### 📋 実験内容
5つの異なる LLM モデルを LiteLLM プロキシ経由で Claude Code Actions に接続し、同一タスク（ワークフローのコードレビュー）を実行して比較評価を行いました。
...
| モデル名 | 評価 | Issue | 実行状態 | 主な問題 | 実用性 |
|---------|------|-------|---------|---------|--------|
| **gpt-5-mini** | ⭐⭐⭐⭐✨ (4.5/5) | #6 | ✅ 成功 | 内部思考の露出 | **最も実用的** |
| **gpt-oss-120b** | ⭐⭐⭐ (3/5) | #5 | ✅ 成功 | 表面的な分析 | 実用可能 |
| **gpt-5-nano** | ⭐⭐ (2/5) | #8 | ⚠️ 部分的 | 57ターンの無限ループ | 重大な問題 |
...
| モデル | ターン数 | 実行時間 | 出力品質 | 安定性 |
|--------|---------|---------|---------|--------|
| gpt-5-mini | 1 | 〜30秒 | 高 | 高 |
| gpt-oss-120b | 1 | 〜20秒 | 中 | 高 |
| gpt-5-nano | 57 | 〜5分 | 低 | 低 |
| qwen3-coder | 1 | 〜10秒 | なし | 失敗 |
| gpt-5 | - | タイムアウト | なし | 失敗 |
...
2. **モデル特性の違い**
   - **gpt-5-mini**: 詳細な分析力と実用性のバランスが最良
   - **gpt-oss-120b**: 安定性重視だが深さに欠ける
   - **gpt-5-nano**: 過度に複雑な思考プロセスで実用性に問題
...
このレポジトリは LiteLLM と Claude Code Actions の統合における**リファレンス実装**として機能します。新しいモデルや設定の検証に活用してください。
